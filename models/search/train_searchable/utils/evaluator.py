...
class CocoEvaluator(Evaluator):
    ...
    def __init__(self, dataset,logger, distributed=False, pred_yxyx=False,):
...
class COCOeval:
    ...
    def summarize(self,logger):
        ...
        '''
        def _summarize( logger, ap=1, iouThr=None, areaRng='all', maxDets=100 ):
            p = self.params
            ...
            return mean_s
        def _summarizeDets(logger):
            stats = np.zeros((12,))
            stats[0] = _summarize(logger,1)
            stats[1] = _summarize(logger,1, iouThr=.5, maxDets=self.params.maxDets[2])
            stats[2] = _summarize(logger,1, iouThr=.75, maxDets=self.params.maxDets[2])
            stats[3] = _summarize(logger,1, areaRng='small', maxDets=self.params.maxDets[2])
            stats[4] = _summarize(logger,1, areaRng='medium', maxDets=self.params.maxDets[2])
            stats[5] = _summarize(logger,1, areaRng='large', maxDets=self.params.maxDets[2])
            stats[6] = _summarize(logger,0, maxDets=self.params.maxDets[0])
            stats[7] = _summarize(logger,0, maxDets=self.params.maxDets[1])
            stats[8] = _summarize(logger,0, maxDets=self.params.maxDets[2])
            stats[9] = _summarize(logger,0,areaRng='small', maxDets=self.params.maxDets[2])
            stats[10] = _summarize(logger,0, areaRng='medium', maxDets=self.params.maxDets[2])
            stats[11] = _summarize(logger,0, areaRng='large', maxDets=self.params.maxDets[2])
            return stats
        def _summarizeKps():
            stats = np.zeros((10,))
